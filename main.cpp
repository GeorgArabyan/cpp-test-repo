...
class SearchServer {
   public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        // при равных релевансах сортируем по рейтингам
    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        return FindTopDocuments(raw_query,
                                [status](int, DocumentStatus what_status, int) {
                                    return what_status == status;
                                });
        // пишем без имён             ~~~~~^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^~~~~,
        // т.к. не используем в лямбде
    }
    ...
    vector<Document> FindAllDocuments(const Query& query, Filter filter) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto& [document_id, term_freq] :
                ...
                if (filter(document_id, documents_.at(document_id).status,
...
int main() {
    ...
    // ПЕРЕДАЁМ ЛЯМБДУ-ПРЕДИКАТ В FindTopDocuments ВТОРЫМ АРГУМЕНТОМ
    for (const Document& document : search_server.FindTopDocuments(
